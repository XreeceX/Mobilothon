...
### add_data.py
 ...
 3. To unlock a car, send a POST request to `/unlock` with a JSON payload containing `user_id` and `car_id`.
    
 4. To lock a car, send a POST request to `/lock` with a JSON payload containing `user_id` and `car_id`.
